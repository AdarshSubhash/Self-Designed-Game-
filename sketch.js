...
how1=loadImage("Images/Settings_BTN.png")
...
Money.visible=false;
m1=createSprite(displayWidth/1.656,displayHeight/2.49,displayWidth/8.2,displayHeight/3.4);
m1.visible=false;
...
dist22.visible=false;
dist11.visible=false;
dist33.visible=false;
how=createSprite(displayWidth/1.07,displayHeight/16,displayWidth/11.2,displayHeight/11);
...
textSize(30);
 ...
 dist2.shapeColor="red";
  ...
  if(gameState === PLAY){
    ...
    if(keyDown(RIGHT_ARROW)){
      ground.velocityX=-15;
      fuel = fuel -  Math.round(getFrameRate()/61);
 ...
 hf=hf+5;
        ...
        if(dist.x>displayWidth/1.601){
          text("YOU HAVE COMPLETED THE GAME",displayWidth/2,displayHeight/2);
          text("PART 2 COMING SOON",displayWidth/2,displayHeight/3)
          ground.velocityX=0;
          dist2.velocityX=0;
          dist.velocityX=0;
           };
  ...
  //text("", displayWidth/1.84, displayHeight/1.97);
   ...
   // monstersGroup.setVelocityXEach(0);
    ...
    function reset(){
      ...
      health=5;
      fuel=25;
      dist.x=displayWidth/2.655;
      dist2.x=displayWidth/2.655;
      
 }
 function howto(){
   fill("blue");
text("=> Use Right Arrow Key To Move And Leave To Stop Moving",displayWidth/2.7,displayHeight/7);
fill("green");
text("$ Collect Money After Every Ride And Add Fuel And Health ",displayWidth/2.5,displayHeight/5);
fill("red");
text("! Complete Your Journey To Win",displayWidth/2.5,displayHeight/4);
 }
